...
*	});
************************************************************************ */
const popmodal = (element, options) => {
	let methods = {},
		el = element,
		el_idValue = el.id,
		defaults = {
	...
	methods.initVar = () => {
		...
		el.vars = {
			...
			pop_ev : null,
			pop_prev : null,
			pop_close : el.querySelector(el.options.selector_close),
		}
	}

	/* -----------------------------------------------------------------
		popmodal.initEvents : 이벤트 선언
	-------------------------------------------------------------------- */
	methods.initEvents = () => {
		el.vars.pop = el;

		//접근성 관련 태그 추가
		if(el.options.is_accessibility){
			let _title = el.vars.pop.querySelector('h1').innerHTML;
			el.vars.pop.setAttribute('role','dialog');
			el.vars.pop.setAttribute('aria-label',_title);
			el.vars.pop.setAttribute('aria-modal', true);
			el.vars.pop.setAttribute('aria-hidden',true);
		}

		//클릭이벤트
		let btn = document.querySelectorAll(`[href="#${el_idValue}"], [aria-controls="${el_idValue}"]`)
		...
		//페이지로드
		if(el.options.load_display) methods.popShow();
	}
	...
	methods.popShow = () => {
		...
		//클릭한 객체의 부모가 팝업일 경우
		if(!el.options.load_display && !el.options.display_ajax){
			if(!!el.vars.pop_ev.closest(`.${el.options.class_open}`)){
				pop_prev = el.vars.pop_ev.closest(`.${el.options.class_open}`);
				pop_prev.classList.add(el.options.class_prev);

				//접근성 초기화
				if(el.options.is_accessibility) pop_prev.setAttribute('aria-hidden',true);
			}
		}

		if(el.options.display_ajax){
			setTimeout(() => {
				el.vars.pop.classList.add(el.options.class_open);
				el.vars.pop_close.focus();
			}, 0);
		}else{
			//클래스명 셋팅
			el.vars.pop.classList.add(el.options.class_open);
		...
		//클릭이벤트
		el.vars.pop_close.addEventListener("click", (e) => {
			//콜백함수
	...
	methods.popHide = () => {
		...
		el.vars.pop.classList.add(el.options.class_close);
		setTimeout(() => {
			el.vars.pop.classList.remove(el.options.class_close);
			if(el.options.display_ajax) el.vars.pop.remove();
		}, 400);

		//초점이동 및 클릭한 객체의 부모가 팝업일 경우
		if(!!el.vars.pop_ev && !el.options.display_ajax){
			if(el.vars.pop_ev.closest(`.${el.options.class_open}`)){
				pop_prev = el.vars.pop_ev.closest(`.${el.options.class_open}`);
				pop_prev.classList.remove(el.options.class_prev);

				//접근성 : 접근가능한 상태로 변경
				if(el.options.is_accessibility){
					pop_prev.setAttribute('aria-hidden',false);
					el.vars.pop.setAttribute('aria-hidden',true);
				}
			}else{
				//접근성 초기화
	...
	/* -----------------------------------------------------------------
		popmodal.outputOpen : 외부에서 팝업열기
	-------------------------------------------------------------------- */
	el.outputOpen = (target) => {
		el.vars.pop_ev = target;
		methods.popShow();
	}

	/* -----------------------------------------------------------------
		popmodal.outputOpen : 외부에서 팝업닫기
	-------------------------------------------------------------------- */
	el.outputClose = (target) => {
		if(target) el.vars.pop_ev = target;
		methods.popHide();
	}
...
************************************************************************ */
/* -----------------------------------------------------------------
	ui_setAddID(name) : ID 추가
-------------------------------------------------------------------- */
const ui_setAddID = (name) => {
...
-------------------------------------------------------------------- */
const ui_setState = () => {
	if(document.querySelectorAll('.guide-table1[data-sort]').length > 0){
		let _all = document.querySelectorAll('.guide-table1[data-sort] > table > tbody > tr').length;
		let _wait = document.querySelectorAll('.guide-table1[data-sort] > table > tbody > tr.wait').length;
		let _ing = document.querySelectorAll('.guide-table1[data-sort] > table > tbody > tr.ing').length;
		let _end = document.querySelectorAll('.guide-table1[data-sort] > table > tbody > tr:not([class]) > td.end:not(:empty)').length;
		let _per = ((_end/_all) * 100).toFixed(2);

		document.querySelector('[data-state="all"] span').innerHTML = _all;
		document.querySelector('[data-state="wait"] span').innerHTML = _wait;
		document.querySelector('[data-state="ing"] span').innerHTML = _ing;
		document.querySelector('[data-state="end"] span').innerHTML = _end;
		document.querySelector('[data-state="end"] strong').innerHTML = `(${_per}%)`;

		document.querySelectorAll('.guide-table1[data-sort] > table > tbody > tr > td:first-child').forEach((item, i) => item.innerHTML = i+1);
	}
}

/* -----------------------------------------------------------------
	ui_setModify() : 수정내역 셋팅
-------------------------------------------------------------------- */
const ui_setModify = () => {
	const _table = document.querySelectorAll('.guide-table2 > table');

	_table.forEach((item, i) => {
		let _html = item.outerHTML;
		let _id = ui_setAddID('uiPOPUPMODIFY'+i)
		let _date = item.querySelector('tbody > tr:first-child > td:last-child').innerHTML;
		let _td = item.closest('td.modify');

		//팝업호출객체 생성
		_td.insertAdjacentHTML('afterbegin',`<a href="#${_id}">${_date}</a>`);

		//팝업호출
		_td.querySelector(`[href="#${_id}"]`).addEventListener('click', (e) => {
			e.preventDefault();

			let _pop = document.createElement('div');

			_pop.setAttribute('id',_id);
			_pop.setAttribute('class', 'pop_modal1')
			_pop.innerHTML = `<article>
				<header>
					<h1>수정내역</h1>
					<button type="button" data-btn="close">수정내역 닫기</button>
				</header>
				<div class="pop_cont"><div class="guide-table2">${_html}</div></div>
			</article>`;

			document.body.append(_pop);

			popmodal(document.getElementById(_id),{display_ajax:true}).outputOpen(this);
		});
	});
}
